...
const timeLimitElmnt = document.getElementById("timeLimit") as HTMLInputElement;
const setLimitElmnt = document.getElementById("setLimit")! as HTMLButtonElement;
const timeLeftElmnt = document.getElementById("timeLeftCounter")!;
const excludeNavElmnt = document.getElementById("excludeNav")! as HTMLInputElement;
const excludeShortsElmnt = document.getElementById("excludeShorts")! as HTMLInputElement;
const subscriptionModeElmnt = document.getElementById("subscriptionMode")! as HTMLButtonElement;
const subModeDescriptionElmnt = document.getElementById("subModeDescription")!;
const enableListElmnt = document.getElementById("list")! as HTMLInputElement;
const channelNameElmnt = document.getElementById("channelName")! as HTMLInputElement;
const addChannelElmnt = document.getElementById("addChannel")! as HTMLButtonElement;
const channelListElmnt = document.getElementById("channelList")!; 
const listModeElmnt = document.getElementById("listMode")!as HTMLButtonElement;

const enableScheduleElmnt = document.getElementById("enableSchedule")! as HTMLInputElement;
const scheduleModeElmnt = document.getElementById("scheduleMode")! as HTMLButtonElement;
const schdlModeDescriptionElmnt = document.getElementById("schdlModeDescription")!;
const weekDaySelectionElmnt = document.getElementById("weekDay")! as HTMLSelectElement;
const startTimeElmnt = document.getElementById("startTime")! as HTMLInputElement;
const endTimeElmnt = document.getElementById("endTime")! as HTMLInputElement;
const addToScheduleElmnt = document.getElementById("addToSchedule")! as HTMLButtonElement;
const sundayScheduleElmnt = document.getElementById("sundaySchedule")! as HTMLDivElement;
const mondayScheduleElmnt = document.getElementById("mondaySchedule")! as HTMLDivElement;
const tuesdayScheduleElmnt = document.getElementById("tuesdaySchedule")! as HTMLDivElement;
const wednesdayScheduleElmnt = document.getElementById("wednesdaySchedule")! as HTMLDivElement;
const thursdayScheduleElmnt = document.getElementById("thursdaySchedule")! as HTMLDivElement;
const fridayScheduleElmnt = document.getElementById("fridaySchedule")! as HTMLDivElement;
const saturdayScheduleElmnt = document.getElementById("saturdaySchedule")! as HTMLDivElement;


//setting functionalities-----------------------------------------------------------------------------------------

setLimitElmnt.addEventListener("click", () => {
    if(!timeLimitElmnt.value ||  isNaN(parseInt(timeLimitElmnt.value))) alert("Time limit has to be a valid number");
    else {
        const value = parseInt(timeLimitElmnt.value)
        if(value < 1) alert("Value must be greater than 0");
        else {
...
addChannelElmnt.addEventListener("click", () => {
    ...
    chrome.storage.local.get(["configs"]).then((result) => {
        ...
        if(channels.some((channel: string) => channel == toAdd)) alert("This channel is already in the list!");
        else {
            channels.push(toAdd);
...
addToScheduleElmnt.addEventListener("click", () => {
    ...
    addSchedulePeriod({...endObj, ...startObj}, weekDayName);
})
...
function saveConfig(configObj: Object){
...
function styleButton(mode: string){
    switch(mode){
        case "neutral":
...
function removeChannel(name: string){
    chrome.storage.local.get(["configs"]).then((result: {[result: string]: ConfigsObj}) => {
        const channels = result.configs.channelList.filter((channel: string) => channel != name);
        saveConfig({channelList: channels});
        renderChannels(channels);
    });
}
...
function createScheduleElements(period: SchedulePeriod, weekDay: WeekDays){
...
function removeSchedulePeriod(period: SchedulePeriod, weekDay: WeekDays){
    chrome.storage.local.get(["configs"]).then((result: {[result: string]: ConfigsObj}) => {
        ...
        const newSchedule = {schedule: {...result.configs.schedule, [weekDay]: newDaySchedule}};
        saveConfig(newSchedule);
        renderSchedule(newSchedule.schedule);
    });
}

function addSchedulePeriod(period: SchedulePeriod, weekDay: WeekDays){
    chrome.storage.local.get(["configs"]).then((result: {[result: string]: ConfigsObj}) => {
