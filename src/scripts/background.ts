...
const initialConfigObj = {
    configs: {
        dailyLimit: 1800,  //30min = 1800 seg
        excludeNav: true,
        excludeShorts: false,
...
async function setTime(newTime: number) {
...
let configChecker: number | undefined;
function startChecker(){
    if(!configChecker) configChecker = setInterval(() => {
        manageTimer();
    }, 15000)       //15s
}
...
function manageTimer(){
    ...
    while(!foundEnabler && i < tabs.length){
        ...
        } else switch(configs.subscriptionMode){
            case "neutral":
                startBySubscribed = true;
            break;
        ...
        if(!configs.scheduleEnabled) startBySchedule = true;
        else {
            startChecker();
            const currentDate = new Date(Date.now());
            let currentWeekDay: WeekDays;
            switch(currentDate.getDay()){
                case 0: currentWeekDay = "sunday"; break;
                case 1: currentWeekDay = "monday";  break;
                case 2: currentWeekDay = "tuesday";  break;
                case 3: currentWeekDay = "wednesday";  break;
                case 4: currentWeekDay = "thursday";  break;
                case 5: currentWeekDay = "friday";  break;
                case 6: currentWeekDay = "saturday";  break;
                default: currentWeekDay = "sunday";  break;
            }
            ...
            if(configs.scheduleMode === "whitelist"){
                if(!inSchedule) startBySchedule = true;
            } else {
                if(inSchedule) startBySchedule = true;
            }
        }
...
function handleLoadInfo(message: {type: string, action: string | null, data: TabInfoObj}, sender: chrome.runtime.MessageSender){
    const tabInfo: TabInfoObj = {
        ...message.data,
        id: sender.tab!.id!
    }
...
function handleChangeInfo(message: {type: string, action: string | null, data: TabInfoObj}, sender: chrome.runtime.MessageSender){
    const tabInfo: TabInfoObj = {
        ...message.data,
    }
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    ...
    if (changeInfo.status === 'complete' && tab.url.includes("youtube.com")) {
        if(isTimeUp) {blockTab(tabId); return}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if(message.type == "query" && message.action == "isCounting") timer ? sendResponse(true) : sendResponse(false);
});

chrome.runtime.onMessage.addListener(message => {
    if(message.type == "configChanged") getConfigs();
})